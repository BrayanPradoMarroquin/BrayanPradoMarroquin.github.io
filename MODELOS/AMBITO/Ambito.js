function Global(_instrucciones, _ambito){
    ...
    var ContadorMain=0;
    for(let i = 0; i < _instrucciones.lenght; i++){
        if(_instrucciones[i].tipo === TIPO_INSTRUCCION.MAIN){
            ...
            if (contStart > 1) {
                ...
                cadena.errores.push({
                    tipo: 'Semántico',
                    error: "No es posible ejecutar más de un metodo inicial (main).",
    ...
    if(ContadorMain==0){
        cadena.cadena = `Error: No se ha encontrado ningun metodo main.\n`;
        cadena.errores.push({
            tipo: 'Semántico',
            error: "No se ha encontrado ningun metodo main.",
            linea: "-",
            columna: "-"
        });
        return cadena;
